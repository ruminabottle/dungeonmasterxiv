...
public class RelinkRequiresApprovalTests
{
    ...
    public void AMatchingParticipantIdAdmitsNobody()
    {
        ...
        var request = coordinator.ReceiveJoinRequest("PRBCD4", new byte[] { 1, 2, 3 }, Now, claim);
    ...
    public void ARelinkRequiresTheSameStepsAsAFirstTimeJoin()
    {
        ...
        var relink = relinking.ReceiveJoinRequest("PRBCD4", new byte[] { 1, 2, 3 }, Now, claim);
        var join = joining.ReceiveJoinRequest("PRBCD6", new byte[] { 1, 2, 3 }, Now, RelinkClaim.None);
    ...
    {
        var (coordinator, claim) = HostWithAReturningPlayer();
        coordinator.ReceiveJoinRequest("PRBCD4", new byte[] { 1, 2, 3 }, Now, claim);

        coordinator.Deny("PRBCD4");
